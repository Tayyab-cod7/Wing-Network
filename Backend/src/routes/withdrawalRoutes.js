...
router.post('/request', auth, async (req, res) => {
    try {
        ...
        // Get user's current balance and enforce cooldown
        ...
        if (user.lastWithdrawalTime) {
            const diff = now - user.lastWithdrawalTime; // ms
            if (diff < COOLDOWN_HOURS * 60 * 60 * 1000) {
                const remaining = COOLDOWN_HOURS * 60 * 60 * 1000 - diff;
                return res.status(400).json({
                    success: false,
                    error: `You can withdraw again in ${Math.ceil(remaining / (60 * 1000))} minutes`,
        ...
        // Deduct amount from user's balance and update lastWithdrawalTime
        user.balance -= amount;
        user.lastWithdrawalTime = now;
        await user.save();
        console.log('Updated user balance and lastWithdrawalTime');
